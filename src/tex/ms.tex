...
We sample 1000 events from the following distribution of parameters detailed in
...
\begin{table}[hbt!]
    ...
    $M_c$ & Chirp mass & $[10, 50] M_\odot$\\
    $q$ & Mass ratio & $[0.5, 1]$\\
    $\chi_1$ & Primary dimensionless spin& $[-0.5, 0.5]$\\
    $\chi_2$ & Secondary dimensionless spin & $[-0.5, 0.5]$\\
    $d_L$ & Luminosity distance & $[300, 2000] \textrm{Mpc}$\\
    $t_c$ & Coalescence time & $[-0.5, 0.5] \textrm{s}$\\
    $\phi_c$ & Coalescence phase & $[0, 2\pi]$\\
    $\cos{\iota}$ & Cosine of inclination angle & $[-1, 1]$\\
    $\psi$ & Polarization angle & $[0, \pi]$\\
    $\alpha$ & Right ascension & $[0, 2\pi]$\\
    $\sin{\delta}$ & Sine of declination & $[-1, 1]$\\

    \hline
    \hline
    \end{tabular}
    \caption{Parameters used in the injection-recovery test.}
...
% Describe the distance between flowMC result and Bilby result using Jensen-Shannon divergence, scipy has a function to compute that in scipy.spatial.distance

For a quantitative comparison between results obtained through our code and other existing tools,
we compute the Jensen-Shannon divergence of the marginalized distribution between our code and \texttt{Bilby}.
The Jensen-Shannon divergence is a symmetric measure of the distance between two probability distributions,
with a value of 0 indicating identical distributions and a value of $\ln{2} \textrm{nat}$ representing
the maximum possible divergence between two distributions.
\kw{Report JSO} 
