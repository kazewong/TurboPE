...
\software{arviz} \citep{arviz_2019},
...
CPU time is hard to benchmark in \software{Jax} due to JIT compilation and
...
carry out these runs using \textsc{parallel Bilby} (\software{pBilby})
...
use of autodifferentiation environments like \software{Jax} to become more
...
relativity
\citep{Coogan:2022qxs,Iacovelli:2022bbs,Iacovelli:2022mbg,ripplepaper}.
...
JIT in \software{Jax} requires the computational graph of a function to be fixed
at compile time, which means the ODEs solving part of EOB is not compatible with
\software{Jax}, so the models in the EOB family may not benefit as much as the
other families.}
