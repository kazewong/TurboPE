...
\begin{abstract}
We present a lightweight, flexible, and high-performance framework for
...
gradient-based algorithms struggle with target distributions that exhibit locally-varying
correlations, since they assume a uniform global structure by construction
...
\mi{can we clarify what it means to learn the ``global landscape'', and how
this jives with the fact that norm flows are supposed to help with the
\emph{local} correlations?}
...
relative weight assigned to each chains will not represent the true target
...
PSDs for the LIGO Hanford, LIGO Livingston and Virgo detectors \cite{} \mi{what
curves did you use?}.
For both injection and recovery, we make use of the \textsc{IMRPhenomD}
waveform \cite{Khan:2015jqa} via the fully-differentiable implementation
presented in the \textsc{ripple} package \cite{}.
...
\mi{cite}} for GW150914 and GW170817 respectively. Running on an Nvidia A100
...
\caption{
    GW150914 posterior computed by our code (\mi{blue}) and \textsc{Bilby} (\mi{orange}).
}
\label{fig:GW150914}
\end{figure}
...
\caption{
    GW170817 posterior computed by our code (\mi{blue}) and \textsc{Bilby} (\mi{orange}).
}
\label{fig:GW170817}
\end{figure}
...
\mi{we should say this in the het like sect}
