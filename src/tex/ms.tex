...
\software{LALInference} \citep{Veitch:2014wba}, \software{PyCBC Inference}
\citep{Biwer:2018osg}, and \software{Bilby}
...
To obtain a reference waveform, we currently use the \software{differential
evolution} algorithm \citep{Storn1997DifferentialE} available in the
\software{scipy} package \citep{2020SciPy-NMeth} to find the waveform parameters
...
other hand, schemes like \software{Jax}
...
implemented within \software{Jax} or a similar framework; this includes the
...
waveforms implemented in \software{ripple} \citep{ripplepaper}.
...
to satisfy the detailed balance condition. We use the package \software{flowMC}
...
\begin{algorithm}
\caption{\software{flowMC} pseudocode}\label{alg:cap}
...
of waveform evaluations unlocks the potential of the \software{flowMC} sampling
algorithm.
...
via the fully-differentiable implementation presented in the \software{ripple}
...
effective sample size using \software{arviz}
...
\begin{figure*}
    ...
    \caption{
      GW150914 posterior computed by our code (blue) and \software{Bilby} (gray).
    }
    \label{fig:GW150914}
...
\caption{
    GW170817 posterior computed by our code (blue) and \software{Bilby} (gray).
}
\label{fig:GW170817}
    \script{plot_GW170817.py}
\end{figure*}

For comparison, we produce equivalent runs with \software{Bilby}, using the same
exact data and priors.  We use the \software{dynesty} sampler
\citep{2020MNRAS.493.3132S,dynesty}, with 1000 live points and other settings as
in the configurations files available on github \url{https://github.com/kazewong/TurboPE/tree/main/src}.  We
carry out these runs using \software{parallel Bilby} (\software{pBilby})
...
consistent with those produced by \software{Bilby}.  For a quantitative
comparison, we compute the Jensen-Shannon divergence between our code and
\software{Bilby} for the marginalized distribution for each parameter. The
...
\begin{table}[hbt!]
    ...
    GW150914 and GW170817 between our code and \software{Bilby}. The bold values
...
%% [MI: the following is too vague, and we always rely on a reasonable prior,
%% so I'm removing it.]
% Both approaches rely on having a somewhat reasonable prior to approach the problem.
...
environments like \software{Jax} to become more prevalent in the waveform
...
current limiting factor.  While \software{Jax}'s JIT compilation drastically
...
\software{Jax} leverages the \textsc{Accelerated Linear Algebra} (XLA) compiler
to take advantage of hardware accelerators, which means that \software{Jax} needs
...
\software{differential evolution} algorithm we currently use has not been
implemented in \software{Jax}, and the \software{Jax} waveform we use is not
compatible with the parallelization scheme in the \software{scipy} library,
...
our pipeline, currently built upon \software{ripple} and \software{flowMC}, on a
