...
sampling,\footnote{A typical PE run with \texttt{Bilby} takes ${>}10^6$
...
To obtain a reference waveform, we currently use the \texttt{differential
...
\begin{algorithm}
\caption{\texttt{flowMC} pseudocode}\label{alg:cap}
...
of the \texttt{flowMC} sampling algorithms.
...
\texttt{differential evolution} algorithm we currently use has not been
...
our pipeline, currently built upon \texttt{ripple} and \texttt{flowMC}, on a
...
We thank Will M.~Farr for illuminating discussions.
