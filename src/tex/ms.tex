...
\cite{LIGOScientific:2014pky,LIGOScientific:2021usb,LIGOScientific:2021djp}, Virgo \cite{VIRGO:2014yos} and KAGRA
...
relativity (GR)
\cite{LIGOScientific:2016lio,LIGOScientific:2018dkp,LIGOScientific:2021sio},
and constrain the properties of nuclear matter \cite{LIGOScientific:2018hze}.
...
evolution \cite{LIGOScientific:2021psn}.
...
\cite{Veitch:2014wba,Ashton:2021anp,Cornish:2021wxy,Islam:2022afg,Roulet:2022kot,Lee:2022jpn,Lange:2018pyp,Wofford:2022ykb}.
...
beyond standard CBCs in GR, such as lensing and deviations from GR.
...
ups relative to traditional tools without compromising generality for efficiency
...
computation. Once we obtained the reference waveform, we can check the accuracy
...
\begin{table}[hbt!]
    ...
    indicate the parameters with the largest JSD. The minimum frequency used for
    analyzing GW150914 and GW170817 are $20 \rm{Hz}$ and $23 \rm{Hz}$,
    respectively.}
    \label{tab:JSD}
    \end{center}
\end{table}
...
and phase\cite{2019PASA...36...10T}. Because of the already reasonable
performance of our sampler thanks to hardware accelerators, time and phase
marginalization are not crucial, as the performance of our implementation is not
significantly impacted by having two extra dimensions. We also do not include
calibration uncertainties in this study because of simplicity reason. On top of that, since we are planning
to deploy this pipeline to perform nearly-online PE, we also need to consider estimating
the PSD on the same timescale, which \cite{Cornish:2021wxy} have proposed
a solution for. We are looking into incorporating analytic marginalization
schemes, calibration uncertainties, and real-time PSD estimation in our code as
well.
