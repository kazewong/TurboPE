...
\texttt{bilby} \kw{cite}. These packages have been tested by a number of groups
and well regarded as the standard tools.

% Emerging tools from different group
While the standard tools can deliver trusted PE results, they are known to be computationally intensive.
The exact amount of time needed to analyze one event depends on the length of the event.
These community tools take about a week to analyze shorter events such as binary black hole (BBH) mergers,
where they can take up to a month on a computer cluster to analyze longer events such as binary neutron star (BNS) mergers.
There are effort
